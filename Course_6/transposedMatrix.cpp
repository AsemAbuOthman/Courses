...
void TempRowMatrix(int matrix[3][3],int tempRowMatrix[3][3], short rows, short columns)
{
	for (short i = 0; i < rows; i++)
	{
		for (short j = 0; j < columns; j++) 
		{
			tempRowMatrix[i][j] = matrix[i][j];
		}

	}
}

void TransposeMatrix(int matrix[3][3], short rows, short columns)
{
	int tempRowMatrix[3][3];
	int tempColumnMatrix[3][3];

	TempRowMatrix(matrix, tempRowMatrix, rows, columns);

	for (short i = 0; i < rows; i++)
	{
		for (short j = 0; j < columns; j++)
		{
			matrix[j][i] = tempRowMatrix[i][j];
		}
	}
}
